...
espeak -w /tmp/pipe.wav "${QUESTION}" | tee espeak.wav | sox - -c 1 -r 16000 -e signed -b 16 -t wav - >> multipart_body.txt
...
# Convert mp3 to wav
sox response.mp3 -c 1 -r 16000 -e signed -b 16 response.wav

# Write speech to text response to response.txt
curl -u 78621bd8-1f4d-4047-bce5-be097d993180:5aN4TRmjSpHm -X POST --data-binary @response.wav -o response.json --header "Content-Type: audio/wav" --header "Transfer-Encoding: chunked" "https://stream.watsonplatform.net/speech-to-text/api/v1/recognize?continuous=true"
